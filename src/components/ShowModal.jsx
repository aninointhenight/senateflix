...
function FilmModal({ show, onClose }) {
  ...
  const embedUrl = getYouTubeEmbedUrl(show)

  // Track view + history on mount
  useEffect(() => {
    addToWatchHistory(show.id)
    supabase.rpc('increment_view', { show_id: show.id }).catch(() => {})
  }, [show.id])
    ...
    <ModalShell onClose={onClose}>
      {embedUrl ? (
        <div className="relative aspect-video bg-black rounded-t-lg overflow-hidden">
          <iframe src={embedUrl} title={show.title} className="w-full h-full"
      ...
      <div className="p-6">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {badgeCfg && <span className={`text-xs px-2 py-0.5 rounded font-semibold ${badgeCfg.bg} ${badgeCfg.text}`}>{badgeCfg.label}</span>}
          {show.categories?.name && <span className="text-xs px-2 py-0.5 rounded border border-gray-600 text-gray-400">{show.categories.name}</span>}
          {show.youtube_start > 0 && <span className="text-xs px-2 py-0.5 rounded border border-gray-700 text-gray-500">▶ starts at {formatTime(show.youtube_start)}</span>}
        </div>
        ...
        {show.tags?.length > 0 && (
          <div className="flex flex-wrap gap-2 pt-4 border-t border-gray-800">
            {show.tags.map(t => <span key={t} className="text-xs text-gray-500 bg-[#1f1f1f] px-2.5 py-1 rounded-full">#{t}</span>)}
          </div>
        )}
      </div>
    </ModalShell>
      ...
      {loadingEps ? (
        ...
        <>
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-gray-800/60 bg-[#1a1a1a]">
            <button onClick={() => setView('list')} className="flex items-center gap-1.5 text-gray-400 hover:text-white text-sm transition-colors">
          ...
          <div className="p-5">
            ...
            {playing.episode.description && <p className="text-gray-300 text-sm leading-relaxed mb-4">{playing.episode.description}</p>}
        ...
        <>
          <div className="p-5 pb-3">
            ...
            {show.description && <p className="text-gray-400 text-sm leading-relaxed line-clamp-2">{show.description}</p>}
          </div>
          <div className="pb-4">
            {seasons.map(season => (
              <SeasonBlock key={season.id} season={season} playingEpisodeId={playing?.episode?.id} onPlay={(ep) => playEpisode(ep, season)} />
            ))}
            ...
            return (
              <div key={ep.id} onClick={() => onPlay(ep)}
                ...
                <div className="flex-1 min-w-0 pt-0.5">
                  ...
                  {ep.description && <p className="text-gray-500 text-xs leading-relaxed line-clamp-2">{ep.description}</p>}
                </div>
              </div>
            )
          })}
