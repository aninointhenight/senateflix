...
// Fire-and-forget view increment — wraps rpc() in async to avoid .catch() issue
async function trackView(showId) {
  try {
    await supabase.rpc('increment_view', { show_id: showId })
  } catch {}
}
...
function FilmModal({ show, onClose }) {
  ...
  // Track view + history on mount (async wrapper avoids .catch() issue)
  useEffect(() => {
...
function SeriesModal({ show, onClose }) {
  ...
  function playEpisode(episode, season) {
    ...
    trackView(show.id) // async wrapper — no .catch() needed
