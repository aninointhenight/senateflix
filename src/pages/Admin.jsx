...
import CustomRowsManager       from '../components/admin/CustomRowsManager'
    ...
    <div className="min-h-screen bg-sf-dark flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        ...
        <div className="bg-[#181818] rounded-lg p-8 shadow-2xl border border-gray-800/50">
          ...
          {authError && (
            <div className="bg-sf-red/10 border border-sf-red/40 text-red-400 text-sm rounded p-3 mb-4">{authError}</div>
          )}
    ...
    <div className="min-h-screen bg-[#0d0d0d]">
      <header className="bg-[#141414] border-b border-gray-800 px-6 h-14 flex items-center justify-between sticky top-0 z-40">
        <div className="flex items-center gap-4">
          <Link to="/" className="font-bebas text-sf-red text-2xl tracking-widest hover:opacity-80 transition-opacity">SENATEFLIX</Link>
          <span className="text-gray-600 text-xs hidden md:block border-l border-gray-700 pl-4">Admin Panel</span>
        </div>
        <div className="flex items-center gap-4">
          <span className="text-gray-600 text-xs hidden md:block truncate max-w-48">{session.user.email}</span>
          <Link to="/" className="text-gray-500 hover:text-white text-sm transition-colors">View Site</Link>
          <button onClick={() => { supabase.auth.signOut(); navigate('/') }}
            className="text-gray-500 hover:text-white text-sm transition-colors">Sign Out</button>
        </div>
      </header>

      <div className="border-b border-gray-800 px-6 bg-[#141414] overflow-x-auto">
        <div className="flex gap-0 min-w-max">
          {TABS.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)}
              className={`px-5 py-3.5 text-sm font-medium transition-colors border-b-2 -mb-px whitespace-nowrap ${
                tab === t.id ? 'border-sf-red text-white' : 'border-transparent text-gray-500 hover:text-gray-300'
              }`}>
              {t.label}
            </button>
          ))}
