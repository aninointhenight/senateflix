  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const [catsRes, showsRes] = await Promise.all([
        ...
        supabase.from('shows').select(SHOWS_SELECT).eq('type', 'film').order('created_at', { ascending: false }),
      ])
  ...
  return (
    <div className="min-h-screen bg-sf-dark">
      ...
      <div className="pt-24 pb-20">
        <h1 className="font-bebas text-5xl md:text-6xl text-white px-4 md:px-12 mb-6">Films</h1>
        {categories.map(cat => {
        ...
        {!shows.length && <p className="text-gray-600 text-center py-20 text-sm">No films yet.</p>}
      </div>
